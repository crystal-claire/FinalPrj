		    				...
		    				if (result.isConfirmed) {
		    		    	  ...
		    		    	  // Delete 완료 후, 띄울 알림창
		      				  Swal.fire({
		        			    	...
		        			    	location.href='user_cscenter_deleteIq.lion?code=' + code;
		        			    });
		  		    	  }
		    			})
				});
		...
		<div class="container">
			<div class="row">
				...
				<div class="col-lg-9 col-md-7">
					...
					<c:forEach var="userInquiryArticle" items="${article }">
					<!-- 질문내용 -->
						...
						<tbody>
							<tr>
								...
								<td><input type="text" class="form-control" id="title"
									value="${userInquiryArticle.title }" readonly></td>
							</tr>
							<tr>
								<!-- 사진이 있다면, 여기서 사진도 가져와줘야 합니다 -->
								<th>내용</th>
								<td><textarea class="form-control" id="content" rows="15" readonly>${userInquiryArticle.inquiry }</textarea></td>
							</tr>
							...
							<tr>
								<!-- 사진이 있다면, 여기서 사진도 가져와줘야 합니다 -->
								<th>답변</th>
								<td><textarea class="form-control" id="content" rows="15" readonly>${userInquiryArticle.answer }</textarea></td>
							</tr>
							</tbody>
					...
					<%-- <c:if test="${empty userInquiryArticle.answer }">
						<!-- if, ① 답변 없는 경우 버튼 -->
					...
					<c:if test="${not empty userInquiryArticle.answer }">
						<!-- if, ② 답변 있는 경우 버튼 -->
						...
						<button type="button" class="btn btn-primary lion-primary-btn listBtn"
								onclick="location.href='user_cscenter_listIq.lion'">목록으로</button>
					</c:if> --%>  
					 
						<c:choose>
							<c:when test="${userInquiryArticle.answer eq null }">
							<!-- if, ① 답변 없는 경우 버튼 --> 
							<button type="button" class="btn btn-outline-primary lion-outline-btn cancelBtn">삭제</button>
							<button type="button" class="btn btn-primary lion-primary-btn saveBtn"
									onclick="location.href='user_cscenter_updateIqArticle.lion?code=${userInquiryArticle.inquiry_code}'">수정</button>
						 	<input type="hidden" id="code" value="${userInquiryArticle.inquiry_code}">  
						 	</c:when>
						 	<c:otherwise>
							<!-- if, ② 답변 있는 경우 버튼 -->
							<span class="inquity-alert-text">답변 완료 문의는 수정 및 삭제가 불가합니다.</span>
